...
- **滑动窗口共享最近完成子批次**：子批次并发池内窗口由语言组共享最近一次完成的批次快照，取代通道独立窗口，避免多数并行批次完全丢失跨段上下文。
- **批量失败拆单兜底**：content 侧多条目批次失败时立即拆为单条送回队列补全，已增量落地的条目不重复请求；原「连续失败降级」分支在多条目云端批次下无法触达，已重构移除。
- **非首屏路径改批量（ADR 0006）**：云端 belowFold 与动态增量翻译改走批量路径，首屏视口内仍保留流式；本地 Ollama 无批量并发收益，保留流式首字。
- **整页性能埋点落盘（ADR 0006）**：新增 `recordPageMetrics` 消息，整页完成指标（总耗时/首屏可读/段数/缓存命中）落盘 `METRICS_STORE`；SW 流式首个 chunk 记录 `streamTtft`；诊断页新增整页完成、首屏可读、流式首字三项 p50 + min~max；`getMetrics` 摘要仅聚合翻译类指标，避免诊断埋点污染成功率与平均延迟。

### Fixed（批量并行加固）

- **空译文不再计为成功**：整页批量落地路径对 `success:true` 但译文为空的响应按失败归一化，控制条完成数、回调渲染与最终结果保持一致。
- **取消/会话竞速防护**：批量响应落地前校验会话 id 与取消状态，取消或新整页会话启动后，迟到成功响应不再把译文写回恢复后的页面。
