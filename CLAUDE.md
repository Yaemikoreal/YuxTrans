...
YuxTrans 是面向深阅读的 AI 翻译工具，「响应速度是生命，翻译准度是底线」。

- **浏览器扩展** (`extension/`) — Manifest V3 划词/整页翻译插件，**唯一产品，独立完整应用，无后端**，Service Worker 直连云端 API 或本地 Ollama。
- Python 包已移除；**安卓端为后续版本 roadmap 选项之一**，设计与实现计划见 `docs/superpowers/`。
...
- **缓存键**：`v3:p1:<modelSlug>:<src>:<tgt>:<style>:<归一化文本>`；归一化仅做 NFC + 去零宽字符 + 折叠空白，**不同标点/引号/全角半角视为不同键**（刻意不做近似命中）。
...
- **供应商配置三处同步**：新增/改云端供应商时，`lib/sw/constants.js`（`API_ENDPOINTS`/`DEFAULT_MODELS`）、`options.js`（`DEFAULT_ENDPOINTS`/`DEFAULT_MODELS` + UI）、`manifest.json`（`host_permissions`）必须同步更新。
...
- **引号风格**：JS 单引号。扩展日志带 `[YuxTrans]` 前缀。
- 扩展常量集中在 `lib/sw/constants.js`（`API_ENDPOINTS`、`DEFAULT_MODELS`、`STYLE_PROMPTS` 等）。
- 提交前运行 `npm test`，确保无回归。

## 其他资源

- `docs/`：`UI_DESIGN_SYSTEM.md`（设计规范）、`PROVIDERS.md`（供应商详情）、`PRODUCT_OPTIMIZATION.md`、`adr/`（架构决策记录）、`superpowers/`（安卓端 spec 与计划，roadmap）
- `scripts/generate_extension_icons.py`：图标生成
