...
- **Page Translation（整页翻译）**：最小翻译单位为**段落**——内容脚本扫描文本节点后按块容器聚合（保留 `nodeSpans` 偏移映射供回写），content 层不按条数硬打包，整体一次 `translateBatch` 由 SW 按字符数装填；超长段落（>4000 字符）按句末标点二次拆分为句级条目（切句含缩写/小数/省略号白名单），缓存粒度随之细化到句。批量请求 system prompt 注入页面上下文（domain+title，仅风格参考），并以滑动窗口（off/150/400 字三档）携带上一批末尾上下文。回写统一段落级（block/inline/replace 三态），支持恢复原文。
...
- **Render Path（渲染路径）**：整页翻译的两种执行路径。**流式路径**逐段 SSE、首字优先，但总时间随段数线性增长（云端固定 4 并发）；**批量路径**多段打包（50 段/请求 × 10 并发）、总时间快但整段译完才显示。当前分流：首屏视口内走流式；云端视口外 belowFold 与整页译完后的动态增量翻译均走批量（见 ADR 0006），本地 Ollama 保留流式（批量对本地无并发收益）--非首屏、用户扫读而非逐字，批量总时间优于逐字阅读感。_Avoid_: 把云端 belowFold 也走流式当成默认（大页面几十秒的根因）。
