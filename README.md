<p align="center">
  <img src="logo/logo.png" width="96" alt="YuxTrans Logo">
</p>
...
以下截图以 LangGraph 官方文档页为例，演示从配置到整页翻译的完整流程。

### 1. 配置供应商档案

在设置页保存「供应商档案」（Provider + API Key + 模型），可存多组一键切换；API Key 仅保存在浏览器本地。

![设置页 - 供应商档案配置](logo/使用样例-设置.png)

### 2. Popup 控制面板

点击工具栏图标打开控制中心：实时连接状态、模型切换、整页翻译、翻译模式（仅译文 / 双语）、流式开关，以及用量与缓存看板。
...
**双语模式**，每句原文后以浅色斜体内联追加译文，保留原排版，底部进度条实时显示批次进度与缓存 / API 命中：
