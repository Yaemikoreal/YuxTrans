...
- **骨是秩序**：组件骨架沿系统视觉秩序——分组卡片即信息层级、「标签左 / 值右」的扫读动线、0.5px 分隔线、一屏唯一实心主动作。Popup 与设置页由此获得清晰的落点，而非一片均匀的灰。
...
系统设置式骨架：侧栏五个模块（服务档案 · 翻译偏好 · 交互与显示 · 数据与存储 · 诊断排障），各配分区色线性图标；设置项收进分组卡片，标签在左、控件在右。凭据仅存本机浏览器。
